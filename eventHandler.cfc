...
<cfcomponent extends="mura.cfobject" output="false">
	...
	--->
  <cfscript>
  function onAPIResponse(m){
      var response=m.event('response');

      if(isStruct(response)){
          structDelete(response,'lastupdateby');
          structDelete(response,'lastupdatebyid');
      }
    ...
    Mura.siteConfig()
      .on('beforeWidgetSave',function(Mura){
        ...
        renderStart=function(Mura){
          //Mura.addToHTMLFootQueue('<script src="https://intuit.swarm-west.blueriver.com/plugins/MXP/remote/beacon/?siteid=intuit"></script>');
        },
